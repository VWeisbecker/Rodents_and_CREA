...
#Create supplementary figure for lm-based allometry residuals
```{r}
# PCA in part B and D
res_lm_PCA <- gm.prcomp(residuals_lm)

#Set up PCA summaries to extract importance
summary_meanPCA_lm_res <- round(summary(res_lm_PCA)$PC.summary[2,],3)*100


pdf(file="../Data/Results/SuppPCA_lm_residuals.pdf", height= 7.15, width = 8.64 )
...
# Make axis labels
# PCs <- res_lm_PCA$pc.summary$importance
# PCx.per <- round(PCs[2, 1] * 100, digits = 1)  # % with 1 decimal
# PCx.lab <- paste("PC1", " (", PCx.per, "%)", sep = "")
# PCy.per <- round(PCs[2, 2] * 100, digits = 1)
# PCy.lab <- paste("PC2", " (", PCy.per, "%)", sep = "")
...
# A) shape Residuals PCA - ecology
par(mar = c(4.5, 4.5, 2, 1))
plot(x = res_lm_PCA$x[, 1],  # no flip for consistency
                 y = -res_lm_PCA$x[, 2],  
                 xlab = paste("PC1 (",summary_meanPCA_lm_res[1],"%)", sep=""), 
                 ylab = paste("PC2 (",summary_meanPCA_lm_res[2],"%)", sep=""), 
                 xlim = xlims,
                 ylim = ylims,
                 col = col.diet.means, 
                 pch = pch.gld.means, 
                 ...
                 asp=TRUE)
text(part.coords[1], part.coords[2], "a", cex = 1.5)
text(title_coords[1],title_coords[2], "shape residual, lm-based (ecology)")

text(res_lm_PCA$x[which(rownames(res_lm_PCA$x)=="Mastacomys_fuscus"),1],-res_lm_PCA$x[which(rownames(res_lm_PCA$x)=="Mastacomys_fuscus"),2], expression(italic( "M. fuscus")), pos=1)

text(res_lm_PCA$x[which(rownames(res_lm_PCA$x)=="Conilurus_penicillatus"),1],-res_lm_PCA$x[which(rownames(res_lm_PCA$x)=="Conilurus_penicillatus"),2], expression(italic( "C. penicill.")), pos=3)

text(res_lm_PCA$x[which(rownames(res_lm_PCA$x)=="Notomys_mitchellii"),1],-res_lm_PCA$x[which(rownames(res_lm_PCA$x)=="Notomys_mitchellii"),2], expression(italic( "Notomys")), pos=3)
...
```{r}
save(mean.PCA, res.PCA, res_lm_PCA, file = "../Data/Processed/04-PCA_data.rda")
```

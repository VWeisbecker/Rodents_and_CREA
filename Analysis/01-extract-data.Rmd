...
library(stringi)     # CRAN v1.8.3    
library(data.table)  # CRAN v1.15.4 
library(wesanderson) # CRAN v0.3.7
...
### Split shape dataset into separate big and small patch datasets; in this code, we only focus on the small patch dataset which landmarks the skull conventionally (i.e. according to sutures).
```{r}
...
The two bilateral GPAs can take some time
...
#run separate GPAs for each module 
```{r}
...
#And a list that is repeatedly called to make sure the order is always the same
empty_module_list<- list(basicran=NULL, molar=NULL, orbital=NULL, rostrum=NULL, vault=NULL)
...
names(modules)== names(empty_module_list)

#list for gpas, overwriting the raw data list
module_gpas <- empty_module_list
...
module_csizes <- empty_module_list
...
module_pairs <- empty_module_list
...
module_bilat <- empty_module_list
...
module_shape <- empty_module_list
...
Now that we have extracted the  datasets and the specimen metadata, we are ready for the next analysis step: 02-calculate-user-error.Rmd. Here we save the R objects we will need for later steps so this script does not have to be run again.
```{r}
save(sm.bY.shape, sm.sp.info, rep.spec,sm.pt.names, empty_module_list, module_shape, file = "../Data/Processed/01-bilat-data-w-ERR.rda")
